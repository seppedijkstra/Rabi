...
laser_pulse = pulse_library.const(length=3e-6, amplitude=0.0, marker=1)
rf_pulse = pulse_library.const(length=4e-6, amplitude=1.0)

with exp.acquire_loop_rt(count=2):
    with exp.section(uid="laser_section"):
        for _ in range(100):  # 100 × 3 µs = 300 µs
            exp.play(signal="laser_signal", pulse=laser_pulse)
        exp.delay(signal="laser_signal", time=1e-6)

    with exp.section(uid="rf_section", play_after="laser_section"):
        exp.play(signal="rf_signal", pulse=rf_pulse)
        exp.delay(signal="rf_signal", time=1e-6)
...
show_pulse_sheet("Amplitude Rabi", compiled_exp, interactive=False)