...
with open('./ALL0001/F0001CH1.CSV', 'r') as csvfile:
    ...
    for row in plots[18+offset:]:
        ...
        channel_1_x.append(float(row[3][:1] + row[3][2:])+time_offset)
...
with open('./ALL0001/F0001CH2.CSV', 'r') as csvfile:
    ...
    for row in plots[18+offset:]:
        ...
        channel_2_x.append(float(row[3][:1] + row[3][2:])+time_offset)
        channel_2_y.append(float(row[4])-0.03)
...
with open('./ALL0002/F0002CH1.CSV', 'r') as csvfile:
    ...
    for row in plots[18:-offset]:
        ...
        read_channel_1_x.append(float(row[3][:1] + row[3][2:])+time_offset)
...
ax2.set_ylabel('Channel 1 Out [V]')
